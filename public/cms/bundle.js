var cms = (function () {
        ...
        bootstrap: el => {
            ...
            el.addEventListener('submit', e => {
                ...
                getEls(el, 'input.date').forEach(inputDate => {
                    data[inputDate.name] = ptBrToCommon(inputDate.value, inputDate.dataset.format == 'DD/MM/YYYY HH:mm');
                });
    ...
    var fileOpen = (accept => new Promise((resolve, reject) => {
        accept = accept || '*';
    ...
    function getMacroInput(list, macro = {}) {
        ...
        switch (macro.type) {
            case MACRO_TYPES.FILE:
                return { tag: 'div', className: 'form-group', children: [macro.textval ? { tag: 'div', className: 'mb-2', children: [macro.textval ? { tag: 'a', attrs: {
                                href: macro.textval,
                                download: macro.name,
                                title: 'Clique para baixar o arquivo'
                            }, textContent: 'Baixar' } : { tag: 'span', textContent: 'Nenhum arquivo selecionado' }] } : { tag: 'span' }, { tag: 'button', className: 'btn btn-primary', textContent: 'Trocar arquivo', on: ['click', () => {
    ...
    var macro = (list => ({
        ...
        children: list.length ? list.map(macroData => ({ tag: 'div', className: 'card mb-5', children: [{ tag: 'div', className: 'card-body', children: [{ tag: 'div', className: 'row mb-4', children: [{ tag: 'div', className: 'col-md-6', children: [{ tag: 'input', className: 'form-control', attrs: { type: 'text', placeholder: 'Nome único', value: macroData.name || '' },
                            ...
                            }] }] }, { tag: 'div', className: 'col-md-6 text-md-right', children: [{ tag: 'select', className: 'form-control', children: [{ tag: 'option', attrs: { value: MACRO_TYPES.TEXT }, textContent: 'Texto pequeno' }, { tag: 'option', attrs: { value: MACRO_TYPES.LONGTEXT }, textContent: 'Texto extenso' }, { tag: 'option', attrs: { value: MACRO_TYPES.IMAGE }, textContent: 'Imagem' }, { tag: 'option', attrs: { value: MACRO_TYPES.FILE }, textContent: 'Arquivo para download' }], on: ['change', e => {
